    ...
    private static function get_formatted_attributes($product) {
        ...
        if ($product->is_type('variation')) {
            ...
            if (!empty($raw_attributes)) {
                foreach ($raw_attributes as $attr_name => $attribute) {
                    $display_value = $product->get_attribute($attr_name);
                    if (empty($display_value)) continue;

                    $display_label = wc_attribute_label($attr_name, $product);
                    $clean_name    = str_replace('pa_', '', $attr_name);

                    if (mb_stripos($clean_name, 'renk') !== false || mb_stripos($clean_name, 'color') !== false || mb_stripos($display_label, 'renk') !== false || mb_stripos($display_label, 'color') !== false) {
                        $formatted['color']       = mb_convert_case($display_value, MB_CASE_TITLE, "UTF-8");
                        $formatted['color_label'] = $display_label;
                    } elseif (mb_stripos($clean_name, 'beden') !== false || mb_stripos($clean_name, 'size') !== false || mb_stripos($clean_name, 'numara') !== false || mb_stripos($display_label, 'beden') !== false || mb_stripos($display_label, 'size') !== false || mb_stripos($display_label, 'numara') !== false) {
                        $formatted['size']  = mb_strtoupper($display_value, "UTF-8");
                        $formatted['label'] = $display_label;
                    } elseif (mb_stripos($clean_name, 'ölçü') !== false || mb_stripos($clean_name, 'olcu') !== false || mb_stripos($clean_name, 'boy') !== false || mb_stripos($display_label, 'ölçü') !== false || mb_stripos($display_label, 'boy') !== false) {
                        $formatted['size']  = mb_strtoupper($display_value, "UTF-8");
                        $formatted['label'] = $display_label;
                    } else {
                        $unmapped[] = [
