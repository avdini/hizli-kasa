...
add_action('rest_api_init', function () {
    $sevk_permission = function () { return hizli_kasa_can_access_app(); };
    register_rest_route('hizli-kasa/v1', '/sevk/olustur', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_olustur', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/kalem-ekle', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_kalem_ekle', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/kalem-sil', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_kalem_sil', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/gonder-onayla', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_gonder_onayla', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/alici-onayla', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_alici_onayla', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/alici-reddet', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_alici_reddet', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/yola-cikart', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_yola_cikart', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/teslim-barkod', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_teslim_barkod', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/teslim-onayla', array('methods' => 'POST', 'callback' => 'hizli_kasa_sevk_teslim_onayla', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/liste', array('methods' => 'GET', 'callback' => 'hizli_kasa_sevk_liste', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/detay/(?P<id>\d+)', array('methods' => 'GET', 'callback' => 'hizli_kasa_sevk_detay', 'permission_callback' => $sevk_permission));

    register_rest_route('hizli-kasa/v1', '/sevk/bekleyen-sayisi', array('methods' => 'GET', 'callback' => 'hizli_kasa_sevk_bekleyen_sayisi', 'permission_callback' => $sevk_permission));
...
function hizli_kasa_sevk_olustur($request) {
    ...
    if (!$kaynak || !$hedef || $kaynak === $hedef) return new WP_Error('invalid_depo', 'Kaynak ve hedef depo seçimi geçersiz.', ['status' => 400]);
    if (!hizli_kasa_can_user_manage_depo($user_id, $kaynak)) return new WP_Error('no_permission', 'Kaynak depoda yönetim yetkiniz yok.', ['status' => 403]);
    if (!hizli_kasa_can_user_view_depo($user_id, $hedef)) return new WP_Error('no_permission', 'Hedef depoyu görüntüleme yetkiniz yok.', ['status' => 403]);
    $tables = hizli_kasa_sevk_tables();
    ...
    if (!$wpdb->insert_id) return new WP_Error('db_error', 'Sevk oluşturulamadı.', ['status' => 500]);
...
function hizli_kasa_sevk_kalem_ekle($request) {
    ...
    if (!$sevk || $sevk->durum !== 'taslak') return new WP_Error('invalid_sevk', 'Sadece taslak sevke ürün eklenebilir.', ['status' => 400]);
    if (!hizli_kasa_sevk_user_can_source($sevk, true)) return new WP_Error('no_permission', 'Bu sevki düzenleme yetkiniz yok.', ['status' => 403]);
    $product = hizli_kasa_sevk_find_product_by_sku($sku);
    if (!$product) return new WP_Error('not_found', 'Barkod/SKU ile ürün bulunamadı.', ['status' => 404]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_kalem_sil($request) {
    ...
    if (!$sevk || $sevk->durum !== 'taslak' || !hizli_kasa_sevk_user_can_source($sevk, true)) return new WP_Error('no_permission', 'Kalem silme yetkiniz yok.', ['status' => 403]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_gonder_onayla($request) {
    ...
    $data = $request->get_json_params();
    $sevk = hizli_kasa_sevk_get(intval($data['sevk_id'] ?? 0));
    if (!$sevk || $sevk->durum !== 'taslak' || !hizli_kasa_sevk_user_can_source($sevk, true)) return new WP_Error('invalid_state', 'Sevk onaya gönderilemez.', ['status' => 400]);
    if ((float) $sevk->toplam_adet <= 0) return new WP_Error('empty_sevk', 'En az bir ürün eklenmelidir.', ['status' => 400]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_alici_onayla($request) {
    ...
    $data = $request->get_json_params();
    $sevk = hizli_kasa_sevk_get(intval($data['sevk_id'] ?? 0));
    if (!$sevk || $sevk->durum !== 'onay_bekliyor' || !hizli_kasa_sevk_user_can_target($sevk, true)) return new WP_Error('invalid_state', 'Sevk onaylanamaz.', ['status' => 400]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_alici_reddet($request) {
    ...
    $data = $request->get_json_params();
    $sevk = hizli_kasa_sevk_get(intval($data['sevk_id'] ?? 0));
    if (!$sevk || $sevk->durum !== 'onay_bekliyor' || !hizli_kasa_sevk_user_can_target($sevk, true)) return new WP_Error('invalid_state', 'Sevk reddedilemez.', ['status' => 400]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_yola_cikart($request) {
    ...
    $data = $request->get_json_params();
    $sevk = hizli_kasa_sevk_get(intval($data['sevk_id'] ?? 0));
    if (!$sevk || $sevk->durum !== 'onaylandi' || !hizli_kasa_sevk_user_can_source($sevk, true)) return new WP_Error('invalid_state', 'Sevk yola çıkarılamaz.', ['status' => 400]);
    $tables = hizli_kasa_sevk_tables();
...
function hizli_kasa_sevk_teslim_barkod($request) {
    ...
    $sevk = hizli_kasa_sevk_get(intval($data['sevk_id'] ?? 0));
    $sku = sanitize_text_field($data['sku'] ?? '');
    if (!$sevk || !in_array($sevk->durum, ['gonderildi', 'teslim_kontrol', 'uyusmazlik'], true) || !hizli_kasa_sevk_user_can_target($sevk, true)) return new WP_Error('invalid_state', 'Teslim barkodu işlenemez.', ['status' => 400]);
    $tables = hizli_kasa_sevk_tables();
    $item = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$tables['sevk_kalemleri']} WHERE sevk_id = %d AND sku = %s", $sevk->id, $sku));
    if (!$item) return new WP_Error('not_in_list', 'Bu barkod sevk listesinde yok.', ['status' => 404]);
...
function hizli_kasa_sevk_teslim_onayla($request) {
    ...
    if (!$sevk || !in_array($sevk->durum, ['teslim_kontrol', 'uyusmazlik', 'gonderildi'], true) || !hizli_kasa_sevk_user_can_target($sevk, true)) return new WP_Error('invalid_state', 'Teslim onaylanamaz.', ['status' => 400]);
    if (hizli_kasa_sevk_has_mismatch($sevk->id) && !$force) return new WP_Error('mismatch', 'Teslim listesinde uyuşmazlık var.', ['status' => 409]);
    $tables = hizli_kasa_sevk_tables();
    $items = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$tables['sevk_kalemleri']} WHERE sevk_id = %d", $sevk->id));
    foreach ($items as $item) {
        $qty = $item->teslim_alinan_adet === null ? (float) $item->gonderilen_adet : (float) $item->teslim_alinan_adet;
        if ($qty > 0) Hizli_Kasa_Stock_Manager::transfer_in($item->product_id, $item->variation_id, $sevk->hedef_depo_id, $qty, $sevk->id);
    }
...
function hizli_kasa_sevk_liste($request) {
    ...
    if (empty($view_ids)) return ['items' => [], 'stats' => ['total' => 0, 'yolda' => 0, 'bekleyen' => 0, 'tamamlanan' => 0]];
    ...
    if ($scope === 'incoming') $where = "s.hedef_depo_id IN ($ids_ph)";
    if ($scope === 'outgoing') $where = "s.kaynak_depo_id IN ($ids_ph)";
    $durum = sanitize_text_field($request->get_param('durum') ?: '');
    if ($durum && $durum !== 'all') $where .= $wpdb->prepare(" AND s.durum = %s", $durum);
    $date_start = sanitize_text_field($request->get_param('date_start') ?: '');
    $date_end = sanitize_text_field($request->get_param('date_end') ?: '');
    if ($date_start) $where .= $wpdb->prepare(" AND DATE(s.created_at) >= %s", $date_start);
    if ($date_end) $where .= $wpdb->prepare(" AND DATE(s.created_at) <= %s", $date_end);
    ...
    foreach ($stats_rows as $sr) {
        ...
        if (in_array($sr->durum, ['gonderildi', 'teslim_kontrol', 'uyusmazlik'], true)) $stats['yolda'] += (int) $sr->cnt;
        if (in_array($sr->durum, ['onay_bekliyor', 'onaylandi'], true)) $stats['bekleyen'] += (int) $sr->cnt;
        if ($sr->durum === 'tamamlandi') $stats['tamamlanan'] += (int) $sr->cnt;
    }
...
function hizli_kasa_sevk_detay($request) {
    ...
    if (!$sevk || (!hizli_kasa_sevk_user_can_source($sevk, false) && !hizli_kasa_sevk_user_can_target($sevk, false))) return new WP_Error('not_found', 'Sevk bulunamadı.', ['status' => 404]);
...
function hizli_kasa_sevk_bekleyen_sayisi($request) {
    ...
    if (empty($manage_ids)) return ['count' => 0];
