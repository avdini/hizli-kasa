...
add_action('rest_api_init', function () {
    register_rest_route('hizli-kasa/v1', '/gun-sonu-raporu', [
        'methods' => 'GET',
        'callback' => 'hizli_kasa_gun_sonu_raporu',
        'permission_callback' => fn() => hizli_kasa_can_access_app()
    ]);

    register_rest_route('hizli-kasa/v1', '/reports/orders', [
        'methods' => 'GET',
        'callback' => 'hizli_kasa_get_reports_orders',
        'permission_callback' => fn() => hizli_kasa_can_access_app()
    ]);

    register_rest_route('hizli-kasa/v1', '/reports/refunds', [
        'methods' => 'GET',
        'callback' => 'hizli_kasa_get_reports_refunds',
        'permission_callback' => fn() => hizli_kasa_can_access_app()
    ]);

    register_rest_route('hizli-kasa/v1', '/reports/internet-orders', [
        'methods' => 'GET',
        'callback' => 'hizli_kasa_get_reports_internet_orders',
        'permission_callback' => fn() => hizli_kasa_can_access_app()
    ]);

    register_rest_route('hizli-kasa/v1', '/reports/order-receipt/(?P<id>\d+)', [
        'methods' => 'GET',
        'callback' => 'hizli_kasa_get_reports_order_receipt',
        'permission_callback' => fn() => hizli_kasa_can_access_app()
    ]);

    register_rest_route('hizli-kasa/v1', '/reports/day-end-history', [
        'methods' => 'GET',
...
function hizli_kasa_gun_sonu_raporu($request)
{
    ...
    if (empty($kasa_no)) {
        return new WP_Error('missing_param', 'kasa_no parametresi gerekli.', ['status' => 400]);
    }
...
function hizli_kasa_get_reports_order_receipt($request)
{
    ...
    if (!$order) {
        return new WP_Error('no_order', 'Sipariş bulunamadı.', ['status' => 404]);
    }

    if ($order->get_meta('_hizli_kasa_is_refund') === 'yes') {
        return new WP_Error('invalid_order', 'İade siparişi için bu fiş oluşturulamaz.', ['status' => 400]);
    }
    ...
    $payment_adjustments = ['nakit' => 0.0, 'kart' => 0.0, 'iban' => 0.0];
    ...
    foreach ($order->get_items() as $item) {
        ...
        $refunded_qty = $refunded_qty_map[$key] ?? 0.0;
        $refunded_total = $refunded_total_map[$key] ?? 0.0;
...
function hizli_kasa_get_reports_data($request, $report_type = 'pos_orders')
{
    ...
    } elseif ($report_type === 'internet_orders') {
    ...
    try {
        ...
        $orders = is_object($results) && isset($results->orders) ? $results->orders : (is_array($results) ? $results : []);
        ...
        return ['orders' => [], 'error' => $e->getMessage(), 'total' => 0];
    }
    ...
    foreach ($orders as $order) {
        ...
        $has_kasa_no = (bool) $order->get_meta('_hizli_kasa_kasa_no');

        if ($report_type === 'pos_refunds' && (!$order_is_refund || !$has_kasa_no)) {
            continue;
        }
        if ($report_type === 'pos_orders' && ($order_is_refund || !$has_kasa_no)) {
            continue;
        }
        if ($report_type === 'internet_orders' && $has_kasa_no) {
            continue;
        }
