...
if (!function_exists('hk_render_barcode_svg')) {
    function hk_render_barcode_svg($text, $height = 45) {
        $text = (string)$text;
        if ($text === '') return '';
        ...
        $svg = '<svg class="hk-print-barcode-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' . $svg_width . ' ' . $height . '" style="width:100%; max-width:220px; height:' . $height . 'px; display:block; margin:4px auto 0 auto;" data-barcode="' . esc_attr($text) . '">';
        $x = 0;
        $is_bar = true;
        for ($i = 0; $i < strlen($barcode_str); $i++) {
