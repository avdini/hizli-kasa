...
## Değişim Ürünü Görsel ve Büyütme (Preview) Davranışı
1. **Görsel Koruma:** İade sekmesinden kasaya değişim için gönderilen ürünlerde (`_is_exchange_return: true`) ürün görsel URL'si (`item.image`) korunur.
2. **Maske Overlay:** Kasaya eklenen değişim iade satırlarında ürün görselinin üzerinde `🔄` değişim rozet katmanı (overlay mask) görüntülenir.
3. **Görsel Büyütme:** Değişim ürün görseline tıklandığında, kasaya eklenen diğer normal ürünlerde olduğu gibi büyük boyutlu görsel önizleme modali (`openImagePreview`) açılır.
